// Code generated by Wire. DO NOT EDIT.

//go:generate go run github.com/google/wire/cmd/wire
//go:build !wireinject
// +build !wireinject

package main

import (
	"cdncloud/internal/biz"
	"cdncloud/internal/biz/logic"
	"cdncloud/internal/conf"
	"cdncloud/internal/data"
	"cdncloud/internal/server"
	"cdncloud/internal/service"
	"cdncloud/internal/service/api"
	"cdncloud/sessions"
	"github.com/go-kratos/kratos/v2"
	"github.com/go-kratos/kratos/v2/log"
)

import (
	_ "go.uber.org/automaxprocs"
)

// Injectors from wire.go:

// wireApp init kratos application.
func wireApp(confServer *conf.Server, confData *conf.Data, logger log.Logger) (*kratos.App, func(), error) {
	db, err := data.NewDataBase(confData)
	if err != nil {
		return nil, nil, err
	}
	dataData, cleanup, err := data.NewData(confData, logger, db)
	if err != nil {
		return nil, nil, err
	}
	greeterRepo := data.NewGreeterRepo(dataData, logger)
	greeterUsecase := biz.NewGreeterUsecase(greeterRepo, logger)
	greeterService := service.NewGreeterService(greeterUsecase)
	grpcServer := server.NewGRPCServer(confServer, greeterService, logger)
	userRepo := data.NewUserRepo(dataData, logger)
	userPersonRepo := data.NewUserPersonRepo(dataData, logger)
	userCompanyRepo := data.NewUserCompanyRepo(dataData, logger)
	sessionHandle := sessions.NewSessionHandle(confData)
	userLogic := logic.NewUserLogic(userRepo, userPersonRepo, userCompanyRepo, sessionHandle)
	userService := api.NewUserService(userLogic)
	workOrderRepo := data.NewWorkOrderRepo(dataData, logger)
	workOrderDetailRepo := data.NewWorkOrderDetailRepo(dataData, logger)
	workOrderLogic := logic.NewWorkOrderLogic(workOrderRepo, workOrderDetailRepo, sessionHandle)
	workOrderService := api.NewWorkOrderService(workOrderLogic)
	httpServer := server.NewHTTPServer(confServer, greeterService, userService, workOrderService, logger)
	app := newApp(logger, grpcServer, httpServer)
	return app, func() {
		cleanup()
	}, nil
}
